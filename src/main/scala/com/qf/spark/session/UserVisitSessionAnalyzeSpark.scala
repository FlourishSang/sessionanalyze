...
import com.qf.sessionanalyze.util.NumberUtils
import com.qf.sessionanalyze.util.ParamUtils
import com.qf.sessionanalyze.util.StringUtils
import com.qf.sessionanalyze.util.ValidUtils
...
import com.qf.spark.session.UserVisitSessionAnalyzeSpark._
...
object UserVisitSessionAnalyzeSpark {
  def main(args: Array[String]): Unit = {
    ...
    //import com.qf.spark.implicits._
    ...
    val sessionId2AggInfoRDD = aggregateBySession(sc, spark, sessionId2ActionRDDCache)
    ...
    var filteredSessionId2AggrInfoRDD = filteredSessionAndAggrStat(sessionId2AggInfoRDD, taskParam, sessionAggrStatAccumulator)
    ...
    // 4、结果的存储
    getTop10Session(top10CategoryList, sessionId2DetailRDD, taskId, sc)
  ...
  /**
    ...
    * @param sc
    */
  def getTop10Session(categoryList: Array[(CategorySortKey, String)],
    ...
    /**
      ...
      */
    var top10CategoryIdList = List[(Long, Long)]()
    for (item <- categoryList) {
      val categoryid = StringUtils.getFieldFromConcatString(item._2, "\\|", Constants.FIELD_CATEGORY_ID).toLong
      top10CategoryIdList = top10CategoryIdList :+ (categoryid, categoryid)
    }
    // 用并行化方式转换为RDD
    val top10CategoryRDD = sc.parallelize(top10CategoryIdList)

    /**
      * 第二步：计算top10品类被各个session点击的次数
      */
    val top10CategorySessionClick = top10CategorySessionClickCount(sessionId2DetailRDD, top10CategoryRDD)
  ...
  /**
    ...
    * @param taskid
    */
  def insertTop10SessionDetail(top10CategorySession: RDD[(String, String)],
                               sessionId2DetailRDD: RDD[(String, Row)],
                               taskid: Long): Unit = {
    //(string,(string, row)))
    val sesstionDetailsRDD = top10CategorySession.join(sessionId2DetailRDD)
  ...
  /**
    ...
    * @return
    */
  def top10CategorySessionClickCount(sessionId2DetailRDD: RDD[(String, Row)], top10CategoryRDD: RDD[(Long, Long)]) = {

    // 将明细数据以sessionId进行分组
    val sessionDetailsRDD = sessionId2DetailRDD.groupByKey()

    // 将品类id对应的session和count数生成格式为：<categoryId, "sessionId,count">
    val catagorySessionClickCount = sessionDetailsRDD.flatMap(tup => {
      ...
      for (item <- categoryCount.entrySet) {
        var categoryId = item.getKey
        val count = item.getValue
        val value = sessionid + "," + count
        list = list :+ (categoryId, value)
      }

      list
    })

    // 获取到top10热门品类被各个session点击的次数: <categoryId, "sessionId,count">
  ...
  /**
    ...
    * @return
    */
  private def getTop10Category(taskid: Long, sessionId2DetailRDD: RDD[(String, Row)]) = {
    /**
    ...
    /**
      ...
      */
    val categoryId2CountRDD = joinCategoryAndDetal(categoryIdRDD, clickCategoryId2CountRDD, orderCategoryId2CountRDD, payCategoryId2CountRDD)
    ...
    /**
      ...
      */
    val sortKeyCountRDD = categoryId2CountRDD.map(tup => {
  ...
  /**
    ...
    * @return
    */
  private def joinCategoryAndDetal(categoryIdRDD: RDD[(Long, Long)], clickCategoryId2CountRDD: RDD[(Long, Long)], orderCategoryId2CountRDD: RDD[(Long, Long)], payCategoryId2CountRDD: RDD[(Long, Long)]) = { // 注意：如果用leftOuterJoin可能出现右边RDD中join过来的值为空的情况
    ...
    var tmpMapRDD = tmpJoinRDD.map(tup => {
      ...
      var clickCount = optional.getOrElse(null)
      if (clickCount == null)
        clickCount = 0L
  ...
  /**
    ...
    * @return
    */
  private def getPayCategoryId2CountRDD(sessionId2DetailRDD: RDD[(String, Row)]) = { // 过滤下单字段为空的数据
  ...
  /**
    ...
    * @return
    */
  private def getOrderCategoryId2CountRDD(sessionId2DetailRDD: RDD[(String, Row)]) = {
    // 过滤下单字段值为空的数据
    val orderActionRDDD = sessionId2DetailRDD.filter(tup => {
      val row = tup._2
    ...
    // 生成元组，便于聚合
    val orderCategoryIdRDD = orderActionRDDD.flatMap(tup => {
  ...
  /**
    ...
    * @return
    */
  private def getClickCategoryId2CountRDD(sessionId2DetailRDD: RDD[(String, Row)]) = { // 把明细数据中的点击品类字段的空字段过滤掉
  ...
  private def randomExtractSession(sc: SparkContext, taskid: Long, filteredSessionId2AggrInfoRDD: RDD[(String, String)], sessionId2DetailRDD: RDD[(String, Row)]): Unit = {
    ...
    for (countEntry <- countMap.entrySet) {
      ...
      // 取出每个小时的count数
      val count = (String.valueOf(countEntry.getValue)).toLong
      // 用来存储<hour, count>
    ...
    for (dateHourCountEntry <- dateHourCountMap.entrySet) {
      ...
      for (hourCountEntry <- hourCountMap.entrySet) {
        val count = hourCountEntry.getValue // 小时
        val hour = hourCountEntry.getKey // 小时对应的count数

        // 计算每小时session数量占当天session数量的比例，乘以要抽取的数量
        // 最后计算出当前小时需要抽取的session数量
        var hourExtractNumber = ((count.toDouble / sessionCount.asInstanceOf[Double]) * extranctNumber).toInt
        ...
        // 生成上面计算出来的随机数，用while判断生成的随机数不能是重复的
        for (i <- 0 until hourExtractNumber) {
          var extractIndex = random.nextInt(count.asInstanceOf[Int]); // 生成随机索引
  ...
  /**
    ...
    * @return
    */
  private def filterCondition(tup: (String, String), parameter: String, sessionAggrStatAccumulator: SessionAggrStatAccumulator): Boolean = {
