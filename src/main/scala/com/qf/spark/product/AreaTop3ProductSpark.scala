...
object AreaTop3ProductSpark {
  def main(args: Array[String]): Unit = {
    ...
    if (task == null) {
      System.out.println("没有获取到对应taskid的信息")
    }
  ...
  /**
    ...
    * @return
    */
  private def getCityId2CityInfoRDD(spark: SparkSession) = { // 构建连接MySQl的配置信息
...
//    val cityId2CityInfoRDD = cityInfoRDD.map(row => (row.getAs[Long](0), row))
  ...
  /**
    ...
    */
  private def generateTempClickProductBasicTable(spark: SparkSession, cityId2ClickActionRDD: RDD[(Long, Row)], cityId2CityInfoRDD: RDD[(Long, Row)]) = { // 将点击行为和城市信息进行关联：join
