...
object AdClickRealTimeStatSpark extends Serializable {
  def main(args: Array[String]): Unit = { // 模板代码
    ...
    val massage = KafkaUtils.createDirectStream[String, String, StringDecoder, StringDecoder](
    ...
    //      然后对黑名单用户进行去重并存储
    dynamicGenerateBlackList(massage)
    ...
    // 根据动态黑名单进行数据过滤
    val filteredAdRealTimeLogDStream = filteredByBlackList(blackListRDD, massage)
  ...
  /**
    ...
    */
  def calculateClickCountByWindow(filteredAdRealTimeLogDStream: DStream[(String, String)]) = {
  ...
  /**
    ...
    * @param massage
    */
  private def dynamicGenerateBlackList(massage: DStream[(String, String)]) = {

    // 把原始日志数据格式更改为：<yyyyMMdd_userId_adId, 1L>
    val dailyUserAdClickDStream = massage.map(tup => {
      // 从tup中获取每一条原始实时日志
      val log = tup._2
  ...
  /**
    ...
    * @return
    */
  private def filteredByBlackList(blackListRDD: RDD[(Long, Boolean)],
                                  massage: DStream[(String, String)]) = {
    // 接收到原始的用户点击行为日志后，根据数据库黑名单，进行实时过滤
    // 使用transform将DStream中的每个batch RDD进行处理，转换为任意其它的RDD
    // 返回的格式为：<userId, tup>  tup=<offset,(timestamp province city userId adId)>
    val filteredAdRealTimeLogDStream = massage.transform(rdd => {
